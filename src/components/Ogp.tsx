...
const Ogp: React.FC<OgpProps> = ({ post }) => {
  ...
  const ogpPublishedTime =
    post.frontmatter?.date != null
      ? dayjs(post.frontmatter?.date).format('YYYY-MM-DDTHH:mm:ss.SSSZ')
      : null
  const ogpModifiedTime =
    post.frontmatter?.updated != null
      ? dayjs(post.frontmatter?.updated).format('YYYY-MM-DDTHH:mm:ss.SSSZ')
      : null
